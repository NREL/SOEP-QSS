...
TEST( VariableInp1Test, Basic )
{
	VariableInp1< Function_sin > u1( "u1" );
	u1.f().c( 0.05 ).s( 0.5 );
	u1.init0();
	u1.init1();
	u1.init_event();
	...
	EXPECT_DOUBLE_EQ( 0.0, u1.qn( 1.0 ) );
	EXPECT_EQ( 0.0, u1.tQ );
	EXPECT_DOUBLE_EQ( std::max( u1.aTol, u1.rTol * 0.0 ) / 0.025, u1.tE );
