...
For now an emulation of these higher derivatives is provided localized to the `Variable::get_poly_*` functions to allow it to be run. This does not exactly replicate the numeric differentiation of the master branch because only one strategy for setting observee variable values is used, so solutions generated by this code cannot be assumed to be very accurate. Once the extended FMI API is available this temporary code will be replaced and the solutions will then be accurate.
